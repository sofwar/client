define([
    ...
    function Engine() {
        ...
        /* BINGO EVENTS */
        self.ws.on('bingo_before_start', function () {
            console.log('GRAPHIC CONTAINER CHANGED');
            self.bingo = true;
            self.trigger('bingo_before_start');
        });

        self.ws.on('bingo_starting', function (bingo_info) {
            //self.gameState = 'BINGO-STARTING';
            self.bingo = true;
            self.trigger('bingo_starting', bingo_info);
        });

        self.ws.on('bingo_started', function (bingo_info) {
            console.log()
            console.log(self.balanceSatoshis);
            self.bingo = true;
            self.trigger('bingo_started', bingo_info);
        });
        self.ws.on('bingo_win', function (bingo_info) {
            //self.gameState = 'STARTED';

            bingo_info.winners.forEach(function (e, i) {
                if (e.username == self.username)
                {
                    console.log('check balance');
                    self.balanceSatoshis += e.amount;
                }
            });

            self.bingo = true;
            self.trigger('bingo_win', bingo_info);
        });

        self.ws.on('bingo_tick', function () {
            self.bingo = true;
            self.trigger('bingo_tick');
        });
